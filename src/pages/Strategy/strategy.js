...
function StrategySelector() {
	...
	function addStrategy() {
		fetch('/api/strategy', {
			...
			.then((data) => {
				setStrategyList([...strategyList, data.data]);
			});
	}
	...
	function deleteStrategy(id) {
		fetch(`/api/strategy/${id}`, {
			...
			.then((data) => {
				setStrategyList(strategyList.filter((strategy) => strategy.id !== id));
			});
	}
	...
	return (
		<div id='strategy-selector-container'>
			...
			<div id='strategy-list' className='list'>
				{strategyList.map((strategy) => (
					<div className='row-container' key={strategy.id}>
						<div>{strategy.name}</div>
						<div>{strategy.description}</div>
						{manageMode && (
							<div className='fp-btn' onClick={() => deleteStrategy(strategy.id)}>
								删除
							</div>
						)}
					</div>
			...
			<div className='row-container'>
				<div id='add-strategy-btn' className='fp-btn' onClick={addStrategy}>
					添加策略
				</div>
				<div id='man-strategy-btn' className='fp-btn' onClick={() => setManageMode(!manageMode)}>
					管理策略
				</div>
			</div>
		</div>
...
function StrategyBacktestResult() {
	return (
