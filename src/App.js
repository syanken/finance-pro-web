import { useEffect, useRef, useState } from 'react';
import { BrowserRouter, Link, NavLink, Route, Routes } from 'react-router-dom';
import './App.css';
import { DoubleList } from './components';
import './styles/global.css';

class TVChart {
	constructor(chartdom, data = []) {
		this.chartdom = chartdom;
		this.paneCount = 3;
		this.stretchFactor = 4;
		this.chart = LightweightCharts.createChart(this.chartdom, { width: 0, height: 0, rightPriceScale: { visible: true }, leftPriceScale: { visible: false }, timeScale: { visible: true }, grid: { vertLines: { visible: false } } });
		window.addEventListener('resize', this.syncResize);
		this.style = { color: '#2196F3', lineWidth: 1 };
		this.data = data;
		for (leti = 0; i < this.paneCount; i++) {
			this.chart.addSeries(this.type['Bar'], this.style, i).setData([]);
			this.chart.panes()[i].setPreserveEmptyPane(true);
		}
		this.chart.panes()[0].setStretchFactor(this.stretchFactor);
	}
	setData(data) {
		this.chart.panes().forEach((pane, i) => {
			pane.getSeries().forEach((series, j) => {
				this.chart.removeSeries(series);
			});
			if (i < data.length) {
				data[i].forEach((series, j) => {
					this.chart.addSeries(this.type[data[i][j].type], this.style, i).setData(data[i][j].data);
				});
			}
		});
	}
	syncResize() {
		this.chart.applyOptions({ width: this.chartdom.clientWidth, height: this.chartdom.clientHeight });
		this.chart.timeScale().fitContent();
	}
	type = { Area: LightweightCharts.AreaSeries, Bar: LightweightCharts.BarSeries, Candlestick: LightweightCharts.CandlestickSeries, Baseline: LightweightCharts.BaselineSeries, Histogram: LightweightCharts.HistogramSeries, Line: LightweightCharts.LineSeries };
}

function useFetchAllList() {
	const [data, setData] = useState([]);
	const [loading, setLoading] = useState(true);

	useEffect(() => {
		fetch('/api/alllist')
			.then((res) => res.json())
			.then((res) => {
				setData(res);
			})
			.finally(() => setLoading(false));
	}, []);

	return { data, loading };
}

function Home() {
	return <h1>Home1</h1>;
}
const convertKlineData = (data) => {
	let prevClose = 0;
	return [
		data.map((item) => {
			const color = () => {
				if (Number(item[2]) > Number(item[1]) || (Number(item[3]) === Number(item[4]) && Number(item[2]) > prevClose)) {
					return '#ef5350';
				} else if (Number(item[2]) < Number(item[1]) || (Number(item[3]) === Number(item[4]) && Number(item[2]) < prevClose)) {
					return '#26a69a';
				} else {
					return 'black';
				}
			};
			prevClose = Number(item[2]);
			return {
				time: String(item[0]),
				open: Number(item[1]),
				high: Number(item[3]),
				low: Number(item[4]),
				close: Number(item[2]),
				color: color(),
				wickColor: color(),
			};
		}),
		data.map((item) => {
			const color = prevClose === null || item[2] >= prevClose ? '#ef5350' : '#26a69a';
			prevClose = Number(item[2]);
			return {
				time: String(item[0]),
				value: Number(item[5]),
				color,
			};
		}),
	];
};

const getKLineData = async (code) => {
	const res = await fetch(`/api/kline?code=${code}`);
	const data = await res.json();
	const klineData = convertKlineData(data.data);
	console.log(klineData);
	return klineData;
};
function MaCalculater(candleData, maLength = 5) {
	const maData = [];
	for (let i = 0; i < candleData.length; i++) {
		if (i < maLength) {
			maData.push({ time: candleData[i].time });
		} else {
			let sum = 0;
			for (let j = 0; j < maLength; j++) {
				sum += candleData[i - j].close;
			}
			const maValue = sum / maLength;
			maData.push({ time: candleData[i].time, value: maValue });
		}
	}
	return maData;
}
function Quote() {
	const { data: allList, loading } = useFetchAllList();

	const chartRef = useRef(null);
	const chartCandlestickRef = useRef(null);
	const chartVolumeRef = useRef(null);
	const chartInstanceRef = useRef(null);
	useEffect(() => {
		if (!chartRef.current) return;
		const chartOptions = {
			width: chartRef.current.clientWidth,
			height: chartRef.current.clientHeight,
			rightPriceScale: { visible: false },
			leftPriceScale: { visible: true },
			timeScale: { visible: true },
			grid: { vertLines: { visible: true } },
			layout: {
				textColor: 'black',
				background: { type: 'solid', color: 'white' },
				panes: {
					separatorColor: '#626262ff',
					separatorHoverColor: 'rgba(255, 0, 0, 0.1)',
					enableResize: false,
				},
			},
			localization: { dateFormat: 'yyyy-MM-dd' },
		};
		const chart = window.LightweightCharts.createChart(chartRef.current, chartOptions);
		chartInstanceRef.current = chart;

		const candlestickSeries = chart.addSeries(window.LightweightCharts.CandlestickSeries, {
			upColor: '#ef5350',
			downColor: '#26a69a',
			borderVisible: false,
			wickUpColor: '#ef5350',
			wickDownColor: '#26a69a',
		});

		const volumeSeries = chart.addSeries(
			window.LightweightCharts.HistogramSeries,
			{
				color: '#26a69a',
				priceFormat: { type: 'volume' },
				priceScaleId: 'left',
				invertScale: false,
			},
			1
		);
		chartCandlestickRef.current = candlestickSeries;
		chartVolumeRef.current = volumeSeries;
		candlestickSeries.setData([]);
		volumeSeries.setData([]);

		chart.panes()[0].setHeight(chartRef.current.clientHeight * 0.8);
		chart.panes().forEach((item) => {
			item.priceScale('left').applyOptions({
				scaleMargins: {
					top: 0,
					bottom: 0,
				},
			});
		});

		// 3. 自适应
		const syncResize = () => {
			chart.applyOptions({
				width: chartRef.current.clientWidth,
				height: chartRef.current.clientHeight,
			});
			// chart.timeScale().fitContent();
		};
		window.addEventListener('resize', syncResize);
		syncResize(); // 初始化一次

		// 4. 清理
		return () => {
			window.removeEventListener('resize', syncResize);
			chart.remove();
		};
	}, []);
	const handleRowClick = async (code) => {
		const klineData = await getKLineData(code);
		if (chartCandlestickRef.current) {
			chartCandlestickRef.current.setData(klineData[0]); // 直接替换数据
			// chartInstanceRef.current.addSeries(window.LightweightCharts.LineSeries, { color: '#2962FF', lineWidth: 1 }, 0).setData(MaCalculater(klineData[0],5));
		}
		if (chartVolumeRef.current) {
			chartVolumeRef.current.setData(klineData[1]); // 直接替换数据
		}
		// chartInstanceRef.current.timeScale().fitContent();
	};
	return (
		<>
			<div className='kline'>
				<div className='watchlist-item-info-container'>
					<div className='watchlist-item-info'></div>
					<div className='setting-button'>
						<svg stroke='currentColor' fill='currentColor' strokeWidth='0' viewBox='0 0 1024 1024' fontSize='18px' height='1em' width='1em' xmlns='http://www.w3.org/2000/svg'>
							<path d='M924.8 625.7l-65.5-56c3.1-19 4.7-38.4 4.7-57.8s-1.6-38.8-4.7-57.8l65.5-56a32.03 32.03 0 0 0 9.3-35.2l-.9-2.6a443.74 443.74 0 0 0-79.7-137.9l-1.8-2.1a32.12 32.12 0 0 0-35.1-9.5l-81.3 28.9c-30-24.6-63.5-44-99.7-57.6l-15.7-85a32.05 32.05 0 0 0-25.8-25.7l-2.7-.5c-52.1-9.4-106.9-9.4-159 0l-2.7.5a32.05 32.05 0 0 0-25.8 25.7l-15.8 85.4a351.86 351.86 0 0 0-99 57.4l-81.9-29.1a32 32 0 0 0-35.1 9.5l-1.8 2.1a446.02 446.02 0 0 0-79.7 137.9l-.9 2.6c-4.5 12.5-.8 26.5 9.3 35.2l66.3 56.6c-3.1 18.8-4.6 38-4.6 57.1 0 19.2 1.5 38.4 4.6 57.1L99 625.5a32.03 32.03 0 0 0-9.3 35.2l.9 2.6c18.1 50.4 44.9 96.9 79.7 137.9l1.8 2.1a32.12 32.12 0 0 0 35.1 9.5l81.9-29.1c29.8 24.5 63.1 43.9 99 57.4l15.8 85.4a32.05 32.05 0 0 0 25.8 25.7l2.7.5a449.4 449.4 0 0 0 159 0l2.7-.5a32.05 32.05 0 0 0 25.8-25.7l15.7-85a350 350 0 0 0 99.7-57.6l81.3 28.9a32 32 0 0 0 35.1-9.5l1.8-2.1c34.8-41.1 61.6-87.5 79.7-137.9l.9-2.6c4.5-12.3.8-26.3-9.3-35zM788.3 465.9c2.5 15.1 3.8 30.6 3.8 46.1s-1.3 31-3.8 46.1l-6.6 40.1 74.7 63.9a370.03 370.03 0 0 1-42.6 73.6L721 702.8l-31.4 25.8c-23.9 19.6-50.5 35-79.3 45.8l-38.1 14.3-17.9 97a377.5 377.5 0 0 1-85 0l-17.9-97.2-37.8-14.5c-28.5-10.8-55-26.2-78.7-45.7l-31.4-25.9-93.4 33.2c-17-22.9-31.2-47.6-42.6-73.6l75.5-64.5-6.5-40c-2.4-14.9-3.7-30.3-3.7-45.5 0-15.3 1.2-30.6 3.7-45.5l6.5-40-75.5-64.5c11.3-26.1 25.6-50.7 42.6-73.6l93.4 33.2 31.4-25.9c23.7-19.5 50.2-34.9 78.7-45.7l37.9-14.3 17.9-97.2c28.1-3.2 56.8-3.2 85 0l17.9 97 38.1 14.3c28.7 10.8 55.4 26.2 79.3 45.8l31.4 25.8 92.8-32.9c17 22.9 31.2 47.6 42.6 73.6L781.8 426l6.5 39.9zM512 326c-97.2 0-176 78.8-176 176s78.8 176 176 176 176-78.8 176-176-78.8-176-176-176zm79.2 255.2A111.6 111.6 0 0 1 512 614c-29.9 0-58-11.7-79.2-32.8A111.6 111.6 0 0 1 400 502c0-29.9 11.7-58 32.8-79.2C454 401.6 482.1 390 512 390c29.9 0 58 11.6 79.2 32.8A111.6 111.6 0 0 1 624 502c0 29.9-11.7 58-32.8 79.2z'></path>
						</svg>
					</div>
				</div>
				<div id='tvChartContainer'>
					<div ref={chartRef} id='chart'></div>
				</div>
			</div>
			<div id='watchlist-box'>
				<div className='watchlist-header'>
					<div>全部列表</div>
					<button className='add-button' id='filterButton'>
						筛选
					</button>
				</div>
				<div className='list'>
					<DoubleList allList={allList} loading={loading} onRowClick={handleRowClick} />
				</div>
			</div>
		</>
	);
}
function Strategy() {
	return <h1>Strategy</h1>;
}
function Backtest() {
	return <h1>Backtest</h1>;
}
function App() {
	return (
		<BrowserRouter>
			<div className='header'>
				<Link to='/' className='header-btn'>
					Finance Pro
				</Link>
				<hr />
				<div className='row-container'>
					<NavLink to='/' className={({ isActive }) => 'header-btn' + (isActive ? ' active' : '')}>
						首页
					</NavLink>
					<NavLink to='/quote' className={({ isActive }) => 'header-btn' + (isActive ? ' active' : '')}>
						行情
					</NavLink>
					<NavLink to='/strategy' className={({ isActive }) => 'header-btn' + (isActive ? ' active' : '')}>
						策略
					</NavLink>
					<NavLink to='/backtest' className={({ isActive }) => 'header-btn' + (isActive ? ' active' : '')}>
						回测
					</NavLink>
				</div>
				<hr />
				<div>
					<button className='header-btn'>交易</button>
				</div>
				<hr />
				<div className='space'></div>
				<hr />
				<div>
					<button className='header-btn'>设置</button>
				</div>
				<hr />
				<div>
					<button className='header-btn'>登录</button>
				</div>
			</div>
			<div className='page'>
				<Routes>
					<Route path='/' element={<Home />} />
					<Route path='/quote' element={<Quote />} />
					<Route path='/strategy' element={<Strategy />} />
					<Route path='/backtest' element={<Backtest />} />
				</Routes>
			</div>
		</BrowserRouter>
	);
}
export default App;
